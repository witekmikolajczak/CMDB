...
import { getAssetTypes, createAssetType, getDepartments, createDepartment, createAsset, getAssets } from '../api/apiClient';
...
const AssetsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch assets using our API client with proper authentication
        try {
          const assetsResponse = await getAssets();
          if (Array.isArray(assetsResponse)) {
            setAssets(assetsResponse as Asset[]);
          } else {
  ...
  const handleAddAsset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        // Use our new createAsset function that has proper authentication
        await createAsset(assetData);
        
        // If successful, refresh the assets list using our API client function with proper authentication
        const assetsResponse = await getAssets();
        setAssets(assetsResponse as Asset[]);
                              ...
                              filteredDepartments.map(dept => (
                                <div 
                                  ...
                                  onClick={() => {
                                    ...
                                    setDepartmentSearch(dept.name); // Set the name in the input field
                                    document.getElementById('departmentDropdown')?.classList.remove('show');
                                  }}
                                >
                                  {dept.name}
