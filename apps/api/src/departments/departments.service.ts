...
export class DepartmentsService {
  ...
  async findByUserId(userId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch departments for user ${userId}: ${error.message}`,
      );
      throw error;
    } finally {
      if (client) client.release();
    ...
    try {
      ...
      try {
        ...
        if (existingAssignments?.rows.length > 0) {
          const alreadyAssignedUsers = existingAssignments.rows.map(
            (row) => row.user_id,
          );
          throw new Error(
            `Some users are already assigned to a department: ${alreadyAssignedUsers.join(', ')}`,
          );
        }
        ...
        await client?.query(insertQuery, [departmentId, userIds]);
      ...
      this.logger.error(
        `Failed to add users to department ${departmentId}: ${error.message}`,
      );
      throw error;
    } finally {
      if (client) client.release();
  ...
  async removeUserFromDepartment(departmentId: string, userId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to remove user from department: ${error.message}`,
      );
      throw error;
    } finally {
      if (client) client.release();
