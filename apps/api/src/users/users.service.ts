...
export class UsersService {
  ...
  async getUsersCountThisWeek() {
    ...
    try {
      ...
      this.logger.error(`Failed to fetch users count for this week: ${error}`);
      throw error;
    } finally {
      if (client) client.release();
    }
  ...
  async getAllUsers() {
    ...
    try {
      ...
      if (
        rolesTableExists?.rows[0].exists &&
    ...
    try {
      ...
      for (const [key, value] of Object.entries(userData)) {
        ...
        if (columnName === 'department_id' && value !== null) {
          try {
            mappedUserData[columnName] =
              typeof value === 'string' ? parseInt(value, 10) : value;
          } catch (error) {
            this.logger.error(
              `Error converting department_id to integer: ${error}`,
            );
        ...
        // Ensure role_id and status_id are integers if they're not null
          ...
          (columnName === 'role_id' || columnName === 'status_id') &&
          value !== null
        ) {
          try {
            mappedUserData[columnName] =
            ...
            this.logger.error(
              `Error converting ${columnName} to integer: ${error}`,
            );
            // If conversion fails, don't include the field in the update
            continue;
          }
      ...
      this.logger.log(
        `Updating user with ID ${userId} with fields: ${updateFields}`,
      );
      this.logger.log(`Values: ${JSON.stringify(values)}`);

      const returningFields =
        'id, username, email, first_name, last_name, role_id, status_id, department_id';
  ...
  async getProfilePicture(
    ...
    try {
      ...
      this.logger.error(
        `Error getting profile picture: ${error.message}`,
        error,
      );
      return null;
    } finally {
  ...
  async deleteUser(userId: string) {
    ...
    try {
      ...
      // Delete user
      await client?.query('DELETE FROM cmdb.users WHERE id = $1', [userId]);
